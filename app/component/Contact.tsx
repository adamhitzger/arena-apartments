         ...
         <div className=" max-w-fit mx-5 my-8 2xl:mx-8 ">
            ...
            <div className="w-full max-h-fit 2xl:flex">
              <div className=" w-full my-3  p-5 border rounded-[25px] bg-white lg:w-2/4 lg:p-10 lg:ml-3 2xl:mr-3 2xl:p-10 2xl:w-2/4 ">
                <form className="2xl:m-5" action="https://formsubmit.co/adam.hitzger@icloud.com" method="POST">
                <div className="text-2xl my-5">
                  <label htmlFor="fullname">Celé jméno</label>
                </div>
                <div className="mb-5 border-b-2 border-solid border-black">
                  <input className=" w-full p-2" type="text" id='fullname' value={formData.fullname} onChange={(e) => setFormData({...formData, fullname: e.target.value})} required/>
                </div>
                <div className="text-2xl my-5">
                  <label htmlFor="email">Email</label>
                </div>
                <div className="mb-5 border-b-2 border-solid border-black">
                  <input className="w-full p-2" type="text" id='email' value={formData.email} onChange={(e) => setFormData({...formData, email: e.target.value})} required/>
                </div>
                <div className="text-2xl my-5">
                  <label htmlFor="phone">Telefonní číslo</label>
                </div>
                <div className="mb-5 border-b-2 border-solid border-black">
                  <input className=" w-full p-2" type="text" id='phone' value={formData.phone} onChange={(e) => setFormData({...formData, phone: e.target.value})} required/>
                </div> 
                <div className="text-2xl my-5">
                  <label htmlFor="msg">Zpráva</label>
                </div>
                <div className="mb-5 border-b-2 border-solid border-black">
                  <textarea className=" w-full p-2" id='msg' value={formData.msg} onChange={(e) => setFormData({...formData, msg: e.target.value})}></textarea>
                </div>
